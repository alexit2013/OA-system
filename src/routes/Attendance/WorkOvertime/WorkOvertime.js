...
export default class WorkOvertime extends PureComponent {
  componentDidMount() {
    this.props.dispatch({
      type: 'attendance/queryAttendances',
      payload: ATTENDANCE_TYPE.WORK_OVERTIME,
    });
  }
  addNotice = () => {
    const target = '/tabs/attendance/work-overtime/add';
    this.props.dispatch(routerRedux.push(target));
  };

  editAttendance = (id) => {
    this.props.dispatch(routerRedux.push({
      pathname: '/tabs/attendance/work-overtime/add',
      id,
    }));
  };
